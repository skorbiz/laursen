...
export const TimelineEntry = ({ entry, index, side }: TimelineEntryProps) => {
  ...
  const getLocalImage = (imageName: string) => {
    return (
      assetImages[imageName] ||
      assetImages["robot-wall.jpg"] ||
      "/placeholder.svg"
    );
  };
  ...
  const cardContent = (
    <div
      className={`bg-card rounded-lg border border-border dark:border-border/70 border-l-4 ${styles.accent} shadow-sm hover:shadow-md transition-shadow duration-300 overflow-hidden`}
    >
      {/* Image */}
      <div
      ...
      >
        <img
          ...
          onError={(e) => {
            e.currentTarget.src =
              assetImages["robot-wall.jpg"] || "/placeholder.svg";
          }}
        />
      </div>
    ...
    <>
      ...
      <div className="md:hidden relative pl-10">
        ...
        <div className="absolute left-4 top-3 -translate-x-1/2 z-10">
          <div
            className={`w-3 h-3 ${styles.dot} rounded-full border-2 shadow-md`}
          />
        </div>
        {/* Connecting line */}
      ...
      <div className={`hidden md:grid md:grid-cols-2 md:gap-x-8 relative`}>
        {/* Left side content */}
        <div className={`${isLeft ? "pr-2" : ""}`}>{isLeft && cardContent}</div>
        ...
        {/* Connecting line */}
        <div
          className={`absolute top-4 h-0.5 bg-timeline-line w-4 ${isLeft ? "right-1/2" : "left-1/2"}`}
        />
